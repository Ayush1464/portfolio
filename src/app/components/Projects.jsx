...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={containerRef} className="py-40 px-6 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative">
        <header className="mb-32 flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <p className="text-gray-600 dark:text-gray-400 max-w-[300px] text-sm uppercase tracking-widest leading-loose font-medium">
  ...
  return (
    ...
    >
      ...
      <div className={`lg:col-span-7 relative ${index % 2 !== 0 ? 'lg:order-2' : ''}`}>
        <div className="relative aspect-[16/10] rounded-[2rem] overflow-hidden bg-gray-900 border border-gray-200/20 dark:border-white/5 shadow-2xl">
          {/* Spotlight Effect */}
          <div 
          ...
          <img 
            src={project.image} 
            className="w-full h-full object-cover scale-110 group-hover:scale-100 transition-transform duration-[1.5s] ease-out opacity-80 group-hover:opacity-100" 
            alt={project.title} 
          />
          
          {/* Category Tag (Floating) */}
          <div className="absolute top-6 left-6 z-20">
            <span className="px-4 py-2 rounded-full bg-black/70 backdrop-blur-md border border-white/20 text-white text-xs font-bold tracking-widest uppercase flex items-center gap-2">
      ...
      <div className="lg:col-span-5 flex flex-col items-start px-4">
        <h3 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white mb-6 group-hover:translate-x-2 transition-transform duration-500">
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-10">
          {project.tags.map(tag => (
            <span key={tag} className="text-xs font-mono text-blue-600 dark:text-blue-400 border border-blue-500/30 px-3 py-1 rounded-md bg-blue-500/10">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="flex items-center gap-6">
          <a href="#" className="flex items-center gap-2 text-gray-900 dark:text-white font-bold group/link">
            VIEW CASE STUDY 
